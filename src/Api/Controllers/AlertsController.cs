...
public class AlertsController(AppDbContext db) : ControllerBase
{
    ...
    private async Task<bool> CanAccessAlertAsync(Alert alert)
    {
        if (User.IsManager() || alert.CreatedByUserId == UserId) return true;
        if (alert.PropertyId is null) return false;

        var wsIds = await db.WorkspaceMembers
            .Where(m => m.UserId == UserId)
            .Select(m => m.WorkspaceId)
            .ToListAsync();

        return await db.RuralProperties.AnyAsync(p =>
            p.Id == alert.PropertyId &&
            (p.OwnerId == UserId || (p.WorkspaceId != null && wsIds.Contains(p.WorkspaceId.Value))));
    }
    ...
    public async Task<IActionResult> GetById(Guid id)
    {
        ...
        if (!await CanAccessAlertAsync(a)) return Forbid();
    ...
    [Authorize(Roles = $"{Api.Roles.Admin},{Api.Roles.Agronomo}")]
