...
public class UsersController(UserManager<IdentityUser> userManager) : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        foreach (var u in users)
        {
            ...
            result.Add(new
            {
                u.Id,
                u.Email,
                Role       = roles.FirstOrDefault(r => r != "User") ?? roles.FirstOrDefault() ?? Api.Roles.Tecnico,
    ...
    public async Task<IActionResult> Create([FromBody] CreateUserRequest req)
    {
        ...
        var role = IsValidRole(req.Role) ? req.Role : Api.Roles.Tecnico;
    ...
    private static bool IsValidRole(string? role) =>
        role is Api.Roles.Admin or Api.Roles.Agronomo or Api.Roles.Tecnico or Api.Roles.Produtor;
}

public record CreateUserRequest(
    [Required] string Email,
    [Required] string Password,
    string Role = Api.Roles.Tecnico
);
